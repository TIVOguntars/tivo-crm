...
BEGIN
  ...
  INSERT INTO crm.audit_events(entity_type, entity_id, action_type, source_type, source_system,
                               before_data, after_data, changed_fields, performed_by_user_id)
...
BEGIN
  ...
  INSERT INTO crm.audit_events(entity_type, entity_id, action_type, source_type, source_system,
                               before_data, performed_by_user_id)
...
CREATE OR REPLACE FUNCTION crm.rpc_task_followup_count(p_task_id uuid)
RETURNS integer
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  WITH RECURSIVE chain(id) AS (
    SELECT p_task_id
    UNION ALL
    SELECT t.id FROM crm.tasks t JOIN chain c ON t.parent_task_id = c.id
  )
  SELECT GREATEST(count(*)::int - 1, 0) FROM chain;
$$;

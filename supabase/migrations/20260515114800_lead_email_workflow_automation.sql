-- Lead Email Workflow Automation v5
-- Creates: email_send_state, views, helpers, planner, pause/resume, trigger,
--          batch (unscheduled), async dispatcher, reconciler, UI RPCs.
-- Seeds:   workflow_steps for getestimate + sketch.
...
----------------------------------------------------------------------
-- 9. Seed workflow_steps (idempotent UPDATE-then-INSERT)
----------------------------------------------------------------------
DO $seed$
DECLARE r record;
BEGIN
  ...
  LOOP
    UPDATE crm.workflow_steps
       SET template_key  = r.template_key,
           step_order    = r.ord,
    ...
    IF NOT FOUND THEN
      INSERT INTO crm.workflow_steps
        (workflow_id, step_key, step_type, responsible_type,
         template_key, step_order, delay_minutes, is_active)
      VALUES (r.workflow_id, r.step_key, 'email', 'system',

...
-- TEMPORARY Track A bridge. Replace with auth.uid() after Real Auth migration.
CREATE OR REPLACE FUNCTION crm.admin_update_profile_mvp(
  p_actor_user_id uuid,
  p_id            uuid,
  p_full_name     text,
  p_email         text,
...
REVOKE ALL ON FUNCTION crm.admin_update_profile_mvp(uuid, uuid, text, text, text, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION crm.admin_update_profile_mvp(uuid, uuid, text, text, text, boolean) TO authenticated;

-- Security hardening for crm.v_tasks_queue_ui
-- 1. Remove auth.users join (linter ERROR 0002_auth_users_exposed)
-- 2. Restrict SELECT to authenticated + service_role only (drop anon)
...
WITH base AS (
  SELECT
    ...
    NULLIF(TRIM(pl.atbildigais), '')             AS action_owner_label,
    t.due_at,
...
REVOKE ALL ON crm.v_tasks_queue_ui FROM anon;
GRANT SELECT ON crm.v_tasks_queue_ui TO authenticated, service_role;